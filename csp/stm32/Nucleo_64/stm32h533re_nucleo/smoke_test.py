"""Exercise the NUCLEO-H533RE GMP Data Link firmware over the ST-Link VCP."""
...
def discover_port() -> str:
    """Find the NUCLEO-H533RE ST-Link virtual COM port."""
    matches = [port.device for port in list_ports.comports() if port.vid == 0x0483 and port.pid == 0x374E]
    if len(matches) != 1:
        raise RuntimeError(f"Expected one NUCLEO-H533RE VCP, found: {matches}")
    return matches[0]
