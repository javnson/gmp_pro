"""Exercise the NUCLEO-U083RC GMP Data Link firmware over the ST-Link VCP."""
...
def discover_port() -> str:
    """Find the NUCLEO-U083RC ST-Link virtual COM port."""
    matches = [port.device for port in list_ports.comports() if port.vid == 0x0483 and port.pid == 0x3752]
    if len(matches) != 1:
        raise RuntimeError(f"Expected one NUCLEO-U083RC VCP, found: {matches}")
    return matches[0]
