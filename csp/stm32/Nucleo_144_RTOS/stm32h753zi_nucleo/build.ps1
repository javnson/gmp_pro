...
if (-not (Test-Path -LiteralPath (Join-Path $boardDir "Core\Src\main.c"))) {
    throw "CubeMX output is missing. Run generate.ps1 before building."
}

& $python (Join-Path $repoRoot "csp\stm32\Nucleo_144_RTOS\tools\validate_ioc.py") $boardDir
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $python (Join-Path $repoRoot "tools\SDPE_v2\sdpe.py") `
    --settings (Join-Path $repoRoot "tools\SDPE_v2\sdpe_settings.json") validate
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Push-Location (Join-Path $boardDir "sdpe_mgr")
try {
    & $python (Join-Path $repoRoot "tools\SDPE_v2\sdpe.py") `
        --settings (Join-Path $repoRoot "tools\SDPE_v2\sdpe_settings.json") `
        generate-project-local sdpe_requirement.json
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}
finally { Pop-Location }

Push-Location (Join-Path $repoRoot "csp\stm32\Nucleo_144_RTOS\src\gmp_src_mgr")
try {
...
cmake -S $boardDir -B $buildDir -G Ninja `
    "-DCMAKE_TOOLCHAIN_FILE=$(Join-Path $boardDir 'cmake\gcc-arm-none-eabi.cmake')" `
