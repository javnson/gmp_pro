...
if (-not (Test-Path -LiteralPath (Join-Path $boardDir "CM7\Core\Src\main.c")) -or
    -not (Test-Path -LiteralPath (Join-Path $boardDir "CM4\Core\Src\main.c"))) {
    throw "Dual-core CubeMX output is missing. Generate both CM7 and CM4 trees first."
}
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $python (Join-Path $repoRoot "tools\SDPE_v2\sdpe.py") --settings (Join-Path $repoRoot "tools\SDPE_v2\sdpe_settings.json") generate-entity nucleo_h755zi_q
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
