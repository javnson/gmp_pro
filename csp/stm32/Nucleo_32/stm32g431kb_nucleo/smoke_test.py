"""Exercise the NUCLEO-G431KB GMP Data Link firmware over the ST-Link VCP."""
...
def discover_port() -> str:
    """Find the NUCLEO-G431KB ST-Link virtual COM port."""
    matches = [port.device for port in list_ports.comports() if port.vid == 0x0483 and port.pid == 0x374E]
    if len(matches) != 1:
        raise RuntimeError(f"Expected one NUCLEO-G431KB VCP, found: {matches}")
    return matches[0]
