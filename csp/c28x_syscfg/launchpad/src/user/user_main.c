...
/** @brief Execute startup work exactly once and then remove this task. */
static gmp_task_status_t user_task_startup_once(gmp_task_t* task)
{
    startup_task_runs++;
    task->is_enabled = 0;
    return GMP_TASK_DONE;
...
/**
 * @brief The three non-blocking tasks required by the LaunchPad demo.
 * @details Startup disables itself after its first run, heartbeat is periodic
 * at 500 ms, and Data Link is serviced every 2 ms.
 */
