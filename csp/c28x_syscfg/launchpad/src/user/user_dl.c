/**
 * @file user_dl.c
 * @brief Data Link services, tunable parameters, memory view, scope and PIL.
 */
...
gmp_scope_service_t scope_service;
#if defined(ENABLE_GMP_DL_PIL_SIM)
gmp_pil_sim_t pil;
#endif
volatile uint32_t datalink_task_runs;
ctl_dsa_trigger_t dsa_trigger;
ctl_dsa_scope_t dsa_scope;
volatile user_dsa_state_t dsa_state;
volatile uint32_t dsa_generation;
volatile uint16_t dsa_history_write;
volatile uint16_t dsa_history_count;
volatile uint16_t dsa_trigger_position_permille = 500U;
volatile fast16_gt dsa_trigger_channel;
volatile ctl_dsa_trigger_option_t dsa_trigger_mode = DSA_TRIGGER_OPTION_RISING_EDGE;
volatile uint32_t dsa_auto_timeout_ms = 1000UL;
volatile parameter_gt dsa_trigger_level;
...
byte_gt memory_window[64];
#if defined(__TI_COMPILER_VERSION__)
#pragma DATA_SECTION(dsa_buffer, "mass_data")
#endif
ctrl_gt dsa_buffer[USER_DSA_DEPTH * USER_DSA_CHANNELS];
#if defined(__TI_COMPILER_VERSION__)
#pragma DATA_SECTION(dsa_history, "mass_data")
#endif
ctrl_gt dsa_history[USER_DSA_DEPTH * USER_DSA_CHANNELS];
