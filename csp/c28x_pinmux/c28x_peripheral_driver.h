...
extern "C"
{
    ...
    // IIC interface
    GMP_STATIC_INLINE
    void gmp_hal_iic_write(iic_halt iic, fast16_gt dev_addr, const byte_gt* data, size_gt length)
    {

    }

    void gmp_hal_iic_write_async(iic_halt iic, fast16_gt dev_addr, const byte_gt *data, size_gt length);

    void gmp_hal_iic_write_mem(iic_halt iic, fast16_gt dev_addr, fast32_gt mem_addr, const byte_gt *data,
                               size_gt length);

    void gmp_hal_iic_write_mem_async(iic_halt iic, fast16_gt dev_addr, fast32_gt mem_addr, fast_gt mem_length,
                                     const byte_gt *data, size_gt length);

    size_gt gmp_hal_iic_read(iic_halt iic, fast16_gt dev_addr, byte_gt *data, size_gt length);

    // size_gt gmp_hal_iic_read_async(iic_halt iic, fast16_gt dev_addr, byte_gt *data, size_gt length);

    size_gt gmp_hal_iic_read_mem(iic_halt iic, fast16_gt dev_addr, fast32_gt mem_addr, fast_gt mem_length,
                                 byte_gt *data, size_gt length);
