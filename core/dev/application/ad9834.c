...
#include <core/dev/source/ad9834.h>
...
ec_gt ad9834_init(ad9834_dev_t* dev, spi_halt bus, float mclk_hz)
{
    ...
    return gmp_hal_spi_write_16b(dev->bus, dev->shadow_ctrl.all);
}
...
ec_gt ad9834_set_frequency(ad9834_dev_t* dev, ad9834_reg_sel_et reg_sel, float freq_hz)
{
    ...
    ec_gt ret = gmp_hal_spi_write_16b(dev->bus, cmd_mask | lsb_14);
    if (ret != GMP_EC_OK)
        return ret;

    return gmp_hal_spi_write_16b(dev->bus, cmd_mask | msb_14);
}
...
ec_gt ad9834_set_phase(ad9834_dev_t* dev, ad9834_reg_sel_et reg_sel, float phase_deg)
{
    ...
    return gmp_hal_spi_write_16b(dev->bus, cmd_mask | phase_reg);
}
