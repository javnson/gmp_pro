...
static void gmp_canopen_sdo_response_begin(const gmp_canopen_sdo_server_t* server,
...
static void gmp_canopen_sdo_abort(gmp_canopen_sdo_server_t* server,
                                  gmp_canopen_frame_t* response,
                                  ...
                                  uint32_t abort_code)
{
    ...
    response->data[3] = (gmp_canopen_octet_t)subindex;
    gmp_canopen_store_le32(&response->data[4], abort_code);
    gmp_canopen_sdo_server_reset(server);
}

static uint32_t gmp_canopen_sdo_abort_for_od(gmp_canopen_od_result_t result,
                                             fast_gt writing)
{
    switch (result)
    {
    case GMP_CANOPEN_OD_NOT_FOUND: return GMP_CANOPEN_SDO_ABORT_NOT_FOUND;
    case GMP_CANOPEN_OD_READ_DENIED: return GMP_CANOPEN_SDO_ABORT_UNSUPPORTED;
    ...
    case GMP_CANOPEN_OD_LENGTH_MISMATCH: return GMP_CANOPEN_SDO_ABORT_TYPE_LENGTH;
    default: return GMP_CANOPEN_SDO_ABORT_GENERAL;
    }
}
                                     ...
                                     gmp_canopen_od_t* dictionary)
{
    if (server == NULL || dictionary == NULL ||
        node_id < GMP_CANOPEN_NODE_ID_MIN || node_id > GMP_CANOPEN_NODE_ID_MAX)
        return 0;
...
static fast_gt gmp_canopen_sdo_initiate_download(
    gmp_canopen_sdo_server_t* server, const gmp_canopen_frame_t* request,
    gmp_canopen_frame_t* response, uint16_t index, uint16_t subindex)
{
    ...
    if ((command & 0x10U) != 0U ||
        ((!expedited || !size_indicated) && (command & 0x0CU) != 0U))
    {
        gmp_canopen_sdo_abort(server, response, index, subindex,
    ...
    {
        gmp_canopen_sdo_abort(server, response, index, subindex,
                              GMP_CANOPEN_SDO_ABORT_NOT_FOUND);
        return 1;
    }
    if (expedited)
    {
    ...
    {
        gmp_canopen_sdo_abort(server, response, index, subindex,
                              GMP_CANOPEN_SDO_ABORT_OUT_OF_MEMORY);
        return 1;
    }
...
static fast_gt gmp_canopen_sdo_download_segment(
    gmp_canopen_sdo_server_t* server, const gmp_canopen_frame_t* request,
    gmp_canopen_frame_t* response)
{
    uint16_t command = (uint16_t)(request->data[0] & 0xFFU);
    ...
    uint32_t index;
    gmp_canopen_od_result_t result;
    if (server->state != GMP_CANOPEN_SDO_SEGMENTED_DOWNLOAD ||
        toggle != server->toggle)
    {
        index = server->entry == NULL ? 0U : server->entry->index;
        gmp_canopen_sdo_abort(server, response, (uint16_t)index,
                              server->entry == NULL ? 0U : server->entry->subindex,
                              GMP_CANOPEN_SDO_ABORT_TOGGLE);
        return 1;
    }
    if (!last && unused != 0U)
    {
        gmp_canopen_sdo_abort(server, response, server->entry->index,
                              server->entry->subindex,
                              GMP_CANOPEN_SDO_ABORT_COMMAND);
        return 1;
    }
    ...
    server->offset += count;
    gmp_canopen_sdo_response_begin(server, response);
    response->data[0] = (gmp_canopen_octet_t)(0x20U | (toggle << 4U));
    if (last)
    {
        ...
        result = gmp_canopen_od_write(server->entry, server->transfer,
                                      server->total_size);
        if (result != GMP_CANOPEN_OD_OK)
        {
            gmp_canopen_sdo_abort(server, response, server->entry->index,
...
static fast_gt gmp_canopen_sdo_initiate_upload(
    gmp_canopen_sdo_server_t* server, gmp_canopen_frame_t* response,
    uint16_t index, uint16_t subindex)
{
    uint32_t size = 0U;
    gmp_canopen_od_result_t result;
    ...
    result = gmp_canopen_od_read(server->entry, server->transfer,
                                 GMP_CANOPEN_SDO_MAX_TRANSFER, &size);
    if (result != GMP_CANOPEN_OD_OK)
    {
        gmp_canopen_sdo_abort(server, response, index, subindex,
                              gmp_canopen_sdo_abort_for_od(result, 0));
        return 1;
    }
    gmp_canopen_sdo_response_begin(server, response);
    gmp_canopen_store_le16(&response->data[1], index);
    ...
    {
        uint32_t byte_index;
        response->data[0] = (gmp_canopen_octet_t)(0x43U | ((4U - size) << 2U));
...
static fast_gt gmp_canopen_sdo_upload_segment(
    gmp_canopen_sdo_server_t* server, const gmp_canopen_frame_t* request,
    gmp_canopen_frame_t* response)
{
    ...
    uint32_t index;
    fast_gt last;
    if (server->state != GMP_CANOPEN_SDO_SEGMENTED_UPLOAD ||
        requested_toggle != server->toggle)
    {
        gmp_canopen_sdo_abort(server, response,
                              server->entry == NULL ? 0U : server->entry->index,
                              server->entry == NULL ? 0U : server->entry->subindex,
                              GMP_CANOPEN_SDO_ABORT_TOGGLE);
        return 1;
    }
    ...
    gmp_canopen_sdo_response_begin(server, response);
    response->data[0] = (gmp_canopen_octet_t)((server->toggle << 4U) |
                                               (unused << 1U) |
                                               (last ? 1U : 0U));
    for (index = 0U; index < count; ++index)
                                       ...
                                       gmp_canopen_frame_t* response)
{
    ...
    if (server == NULL || request == NULL || response == NULL ||
        request->is_extended || request->is_remote || request->dlc != 8U ||
        request->id != GMP_CANOPEN_COB_RSDO + server->node_id)
        return 0;
    ...
    if ((server->state == GMP_CANOPEN_SDO_SEGMENTED_DOWNLOAD &&
        ...
        (server->state == GMP_CANOPEN_SDO_SEGMENTED_UPLOAD &&
         client_command != 3U) ||
        (server->state == GMP_CANOPEN_SDO_IDLE &&
         (client_command == 0U || client_command == 3U)))
    {
        gmp_canopen_sdo_abort(server, response,
                              server->entry == NULL ? index : server->entry->index,
                              server->entry == NULL ? subindex : server->entry->subindex,
                              GMP_CANOPEN_SDO_ABORT_COMMAND);
        return 1;
    }
    if (client_command == 1U)
        return gmp_canopen_sdo_initiate_download(server, request, response,
                                                  index, subindex);
