...
static void process_line(at_device_entity_t* dev)
{
    ...
    uint16_t name_len = strlen(cmd_start);
    ...
    if (cmd)
    {
        ...
        uint16_t args_len = args ? strlen(args) : 0;
...
//        if (status == AT_STATUS_PENDING)
//        {
//            // 仍然未完成，直接退出 Dispatch，暂停从 buffer 读取新数据
//            // 这会阻塞后续 AT 命令的处理，直到当前命令完成，符合预期
//            return;
//        }
//
//        // 命令完成 (OK 或 ERROR)
...
//        dev->pending_args = NULL;
//        dev->line_idx = 0; // 重要：此时才释放 line_buffer
//
//        if (status == AT_STATUS_ERROR)
//        {
//            report_error(dev, AT_ERR_EXEC_FAIL);
//        }
//        // 继续向下执行，处理 buffer 中堆积的数据
//    }
//
//    // ------------------------------------------------
//    // 2. 错误恢复 (Recovery Phase)
//    // ------------------------------------------------
//    if (dev->flag_overwrite == 1)
//    {
//        // RingBuffer 溢出处理：丢弃所有旧数据直到空，重新同步
...
//            if (dev->line_idx > 0)
//            {
//                dev->cmd_buffer[dev->line_idx] = '\0';
//                process_line(dev);
//
//                // 检查是否进入了 Pending 状态
//                if (dev->pending_cmd != NULL)
//                {
//                    return; // 立即退出，保留 cmd_buffer 内容
//                }
//            }
//        }
//        else
...
//            else
//            {
//                dev->flag_overwrite = 2; // Line Overflow
...
//        dev->pending_args = NULL;
//
//        // 【关键点 1】异步命令结束后，彻底清空 buffer，防止残留
...
//    {
//        report_error(dev, AT_ERR_RX_OVERFLOW);
//        while (ringbuf_get_one(&dev->buffer, &byte)); // Drain buffer
//
//        // 【关键点 2】溢出恢复时，彻底清空 buffer
//        memset(dev->cmd_buffer, 0, AT_LINE_MAX_LEN);
//        dev->line_idx = 0;
//
//        dev->flag_overwrite = 0;
...
//                }
//
//                // 【关键点 4】正常命令处理完毕后，清空整个 buffer
//                // 虽然重置 line_idx 逻辑上够了，但为了安全（防止脏数据），建议 memset
//                // 如果对性能极其敏感，可以只 memset 到 dev->line_idx 的长度
//                // memset(dev->cmd_buffer, 0, dev->line_idx + 1);
//                memset(dev->cmd_buffer, 0, AT_LINE_MAX_LEN);
//                dev->line_idx = 0;
//            }
//            // else: 忽略连续的空行 (\r\n 或 \n\n)
...
//                report_error(dev, AT_ERR_LINE_OVERFLOW);
//
//                // 溢出发生，当前 buffer 数据已不可信，建议清空以便观察调试（可选）
//                // memset(dev->cmd_buffer, 0, AT_LINE_MAX_LEN);
//                // dev->line_idx = 0;
//                // 注意：这里不清空也可以，因为 flag_overwrite=2 会导致后续字符被丢弃直到换行
//            }
//        }
//    }
//}
