...
- [Data Link 无界面 Python API](tools/gmp_datalink/datalink_studio/apis/README_CN.md)
  可供测试程序和 AI Agent 发现 Tunable 参数表及内存白名单，执行带类型的参数访问和
  有界内存访问，配置 Scope 触发、连续采集波形并导出 CSV；同时提供
  [英文 API 手册](tools/gmp_datalink/datalink_studio/apis/README.md)。
- 字节寻址 CPU 使用 `tools/gmp_datalink/datalink_studio/run_u8.bat`，16 位寻址
  DSP 使用 `tools/gmp_datalink/datalink_studio/run_u16.bat`；两个入口共用同一套上位机代码。
