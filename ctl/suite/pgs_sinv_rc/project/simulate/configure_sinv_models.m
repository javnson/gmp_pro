...
common_dir = strrep(fullfile(root, '..', '..', 'src'), '\', '/');
mex_dir = strrep(fullfile(root, '..', '..', '..', '..', '..', 'tools', ...
    'gmp_sil', 'udp_helper_v2', 'mdl_asio_helper', 'bin', 'x64', 'Debug'), '\', '/');
callback = sprintf(['addpath(''%s''); addpath(''%s''); addpath(''%s''); ' ...
    'evalin(''base'',''sdpe_pgs_sinv_rc_common_settings_matlab_init; ' ...
    'sdpe_pgs_sinv_rc_simulate_settings_matlab_init;'');'], ...
    common_dir, sdpe_dir, mex_dir);

addpath(fullfile(root, 'sdpe_mgr'));
addpath(fullfile(root, '..', '..', 'src'));
sdpe_pgs_sinv_rc_common_settings_matlab_init;
sdpe_pgs_sinv_rc_simulate_settings_matlab_init;

for k = 1:numel(models)
    model = models{k};
    load_system(fullfile(root, [model '.slx']));
    set_param(model, 'PreLoadFcn', callback, 'InitFcn', callback, ...
