...
{
    uint32_t status = CAN_getInterruptCause(COMM_CAN_BASE);
    ...
    if (status == 1)
    {
        CAN_readMessage(COMM_CAN_BASE, 1, rx_data);
        CAN_clearInterruptStatus(COMM_CAN_BASE, 1);
    ...
    {
        CAN_readMessage(COMM_CAN_BASE, 2, (uint16_t*)recv_content);
...
void send_monitor_data(void)
{
    ...
    CAN_sendMessage(COMM_CAN_BASE, 4, 8, (uint16_t*)tran_content);
    ...
    CAN_sendMessage(COMM_CAN_BASE, 5, 8, (uint16_t*)tran_content);
    ...
    CAN_sendMessage(COMM_CAN_BASE, 6, 8, (uint16_t*)tran_content);
...
//    tran_content[1].i32 = (int32_t)(protection.active_errors);
    CAN_sendMessage(COMM_CAN_BASE, 7, 8, (uint16_t*)tran_content);
}
...
{
    // Send head
    gmp_hal_uart_write(UART_USB_BASE, gmp_dev_dl_get_tx_hw_hdr_ptr(&dl), gmp_dev_dl_get_tx_hw_hdr_size(&dl), 10);

    // Send data body, if necessary
    if (gmp_dev_dl_get_tx_hw_pld_size(&dl) > 0)
    {
        gmp_hal_uart_write(UART_USB_BASE, gmp_dev_dl_get_tx_hw_pld_ptr(&dl), gmp_dev_dl_get_tx_hw_pld_size(&dl), 10);
    }
}
...
void flush_dl_rx_buffer()
{
    ...
    fifoLevel = SCI_getRxFIFOStatus(UART_USB_BASE);

    if (fifoLevel > 0)
    {
        SCI_readCharArray(UART_USB_BASE, (uint16_t*)rxBuf, fifoLevel);
...
interrupt void INT_UART_USB_RX_ISR(void)
{
    ...
    // Deal with overrun
    if (SCI_getRxStatus(UART_USB_BASE) & SCI_RXSTATUS_OVERRUN)
    {
        SCI_clearOverflowStatus(UART_USB_BASE);
    }

    // Clear interrupt flags
    SCI_clearInterruptStatus(UART_USB_BASE, SCI_INT_RXFF);
