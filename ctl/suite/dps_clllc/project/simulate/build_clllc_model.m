...
add_block('spspowerguiLib/powergui',[mdl '/powergui'],'Position',[760 310 850 350]);
...
add_line(mdl,phP.Outport(1),phM.Inport(1),'autorouting','on');
add_line(mdl,phP.Outport(2),phM.Inport(2),'autorouting','on');
add_line(mdl,phP.Outport(4),phM.Inport(3),'autorouting','on');
...
zero=add_block('simulink/Sources/Constant',[sub '/Unused ADC Channels'], ...
    'Value','zeros(19,1)','Position',[930 105 1010 135]);
...
'function [gate,trigger] = fcn(enable,pwm,monitor,t)\n' ...
...
'period_pu=max(monitor(1),0.5); period=period_pu/100000;\n' ...
'period_ticks=1000*period_pu; dead=max(monitor(2),0);\n' ...
'gate=zeros(8,1);\n' ...
'for leg=1:4\n' ...
' duty=min(max(double(pwm(leg))/period_ticks,0),1);\n' ...
' phase=double(pwm(leg+4))/period_ticks; x=mod(t/period-phase,1);\n' ...
' gate(leg)=enable>0.5 && x>=dead && x<max(duty-dead,0);\n' ...
' gate(leg+4)=enable>0.5 && x>=min(duty+dead,1) && x<1-dead;\n' ...
'end\n' ...
'bucket=floor(t/(period*2)); trigger=double(bucket~=previous_bucket); previous_bucket=bucket;\n' ...
'end\n']);
