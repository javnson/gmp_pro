...
* **SVPWM Generation (`svpwm.h`)**: Implements the Space Vector Pulse Width Modulation algorithm, which converts a voltage command vector in the stationary α-β frame into three-phase PWM duty cycles.
...
* **Voltage Reconstruction (`voltage_calculator.h`)**: Reconstructs the three-phase AC voltages and α-β frame voltages applied to the motor based on the DC bus voltage and SVPWM duty cycles.
