...
typedef struct _tag_pmsm_hall_obs_t
{
    ...
    // --- State Variables ---
    data_gt prev_hall_state; //!< Previous 3-bit Hall state (1-6).
 ...
 * @return data_gt State from 1 to 6. Returns 0 or 7 on hardware fault.
 */
GMP_STATIC_INLINE data_gt ctl_get_hall_state(data_gt a, data_gt b, data_gt c)
{
    return (data_gt)((c << 2) | (b << 1) | a);
}
