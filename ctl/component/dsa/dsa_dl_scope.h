...
typedef struct
{
    ...
    uint32_t sample_rate_hz;           /**< Sampling rate before any future divider. */
 ...
 * @note Every non-simulation suite application using the standard platform
 * scope must provide this function.
 */
void user_get_scope_channels(ctrl_gt channels[CTL_DSA_DL_SCOPE_CHANNELS]);

/**
 * @brief Initialize the scope instance owned by a platform peripheral file.
 * @param dl Data Link object used by the suite application.
 */
void xplt_init_dl_scope(gmp_datalink_t* dl);

/**
 * @brief Dispatch a received packet to the platform-owned Scope instance.
 * @return Nonzero when the Scope service handled the packet.
 */
fast_gt xplt_dispatch_dl_scope(void);

/** @brief Sample the platform-owned Scope once after a control step. */
void xplt_step_dl_scope(void);

/**
 * @brief Define the standard four-channel Scope inside one peripheral C file.
 ...
 * both buffers in the peripheral translation unit lets each linker place the
 * acquisition memory according to the target memory map.
 */
#define CTL_DSA_DL_SCOPE_DEFINE_PLATFORM(resource_name)                                      \
    static ctrl_gt xplt_dl_scope_buffer[CTL_DSA_DL_SCOPE_CHANNELS * GMP_DL_SCOPE_DEPTH];      \
    static ctrl_gt xplt_dl_scope_history[CTL_DSA_DL_SCOPE_CHANNELS * GMP_DL_SCOPE_DEPTH];     \
    static ctl_dsa_dl_scope_t xplt_dl_scope;                                                   \
    void xplt_init_dl_scope(gmp_datalink_t* dl)                                                \
    {                                                                                          \
        ctl_init_dsa_dl_scope(&xplt_dl_scope, dl, CTL_DSA_DL_SCOPE_DEFAULT_CMD,                \
                              (resource_name), xplt_dl_scope_buffer,                            \
                              xplt_dl_scope_history, GMP_DL_SCOPE_DEPTH,                        \
                              (uint32_t)(CONTROLLER_FREQUENCY));                                \
    }                                                                                          \
    fast_gt xplt_dispatch_dl_scope(void)                                                       \
    {                                                                                          \
        return ctl_dsa_dl_scope_rx_cb(&xplt_dl_scope);                                         \
    }                                                                                          \
    void xplt_step_dl_scope(void)                                                              \
    {                                                                                          \
        ctrl_gt channels[CTL_DSA_DL_SCOPE_CHANNELS];                                           \
        user_get_scope_channels(channels);                                                     \
        ctl_step_dsa_dl_scope_4ch(&xplt_dl_scope, channels[0], channels[1],                    \
