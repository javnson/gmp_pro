...
/** @brief Return the standard user Scope descriptor for facility registration. */
gmp_dl_facility_t* user_dl_scope_facility(void);
 ...
 * The application may place `user_dl_scope_storage` in a target-specific
 * linker section before invoking this macro. Both generated objects remain
 * global so they are easy to inspect in CCS Expressions.
 */
#define CTL_DSA_DL_SCOPE_DEFINE_USER(resource_name)                                          \
    ctrl_gt user_dl_scope_storage[CTL_DSA_DL_SCOPE_STORAGE_ELEMENTS(                          \
        CTL_DSA_DL_SCOPE_CHANNELS, GMP_DL_SCOPE_DEPTH)];                                      \
    ctl_dsa_dl_scope_t user_dl_scope;                                                          \
    void user_init_dl_scope(gmp_datalink_t* dl)                                                \
    {                                                                                          \
        (void)ctl_init_dsa_dl_scope_workspace(                                                 \
            &user_dl_scope, dl, CTL_DSA_DL_SCOPE_DEFAULT_CMD, (resource_name),                 \
    ...
    gmp_dl_facility_t* user_dl_scope_facility(void)                                            \
    {                                                                                          \
        return ctl_dsa_dl_scope_facility(&user_dl_scope);                                      \
    }                                                                                          \
    void user_step_dl_scope(void)                                                              \
    {                                                                                          \
