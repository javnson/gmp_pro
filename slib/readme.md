...
| `install_path/ctl_simulink_components/<Release>/` | Independent installed CTL component library, MEX directory, and `slblocks.m` | Generated by the CTL component installer; do not merge its registration file with the main library. |
...
6. invokes `ctl_simulink_components/matlab/install_ctl_simulink_components.m` to generate an independent `install_path/ctl_simulink_components/<Release>` tree with its own `slblocks.m` and `mex/` directory;
