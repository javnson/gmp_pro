...
`ctl_simulink_components` 用于把真实的 GMP CTL 元件转换成自动生成、带 Mask 的 Simulink Block。它现在由 `slib` 统一维护，并随 GMP Simulink 主库一起安装和卸载。
...
通常应直接运行 `slib/install_gmp_simulink_lib.m`；主安装程序会自动调用上述组件安装程序。
...
编辑器左侧按分类显示全部元件，右侧可表格化维护多输入/多输出端口、参数分组、外部输入资格，并预览生成的 C++ S-Function。刷新 MATLAB 后可以在 Simulink Library Browser 中找到 **CTL Simulink Components**。
