# CTL Simulink Components

**English** | [简体中文](README_CN.md)

`ctl_simulink_components` turns real GMP CTL components into generated, masked Simulink blocks. It is maintained under `slib` and is installed and uninstalled together with the main GMP Simulink library.
...
```bat
slib\ctl_simulink_components\run_builder.bat
```

Or validate and generate from the command line:

```bat
cd /d %GMP_PRO_LOCATION%\slib\ctl_simulink_components
python ctl_simulink_components.py validate
python ctl_simulink_components.py generate
```

Install for the active MATLAB Release:

```matlab
run(fullfile(getenv('GMP_PRO_LOCATION'), 'slib', ...
    'ctl_simulink_components', 'matlab', ...
    'install_ctl_simulink_components.m'));
```

Normally, run `slib/install_gmp_simulink_lib.m` instead; the main installer calls this component installer automatically.
...
The editor presents all definitions in a category tree. It manages scalar MIMO port tables, parameter groups and external-input eligibility, and previews generated C++ before generation. Open **CTL Simulink Components** in the Simulink Library Browser after installation.
