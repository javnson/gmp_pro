...
def _render_include_path(
    raw_path: str, manager_dir: Path, project_root: Path, gmp_root: Path
) -> str:
    ...
    try:
        ...
        suffix = relative.as_posix()
        return "${CMAKE_CURRENT_LIST_DIR}" + (f"/{suffix}" if suffix != "." else "")
    except ValueError:
...
def generate_cmake(workspace: Path, output: Path | None = None) -> Path:
    ...
    include_paths = {
        _render_include_path(path, manager_dir, project_root, gmp_root)
        for path in raw_include_paths
    }
    for directory_name in LOCAL_SOURCE_DIRS:
        if (project_root / directory_name).is_dir():
            include_paths.add(f"${{CMAKE_CURRENT_LIST_DIR}}/{directory_name}")

    output_path = (output or (project_root / "gmp_config.cmake")).resolve()
