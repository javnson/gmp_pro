...
echo [2/4] Configuring the handwritten Eigen C++ test with GMP's private vcpkg...
...
echo [2/4] Configuring the handwritten Eigen C++ test with system vcpkg...
...
if errorlevel 1 goto :failed
echo [4/4] Running the 48 V, 20 Hz open-loop PMSM drive test...
...
:failed_with_result
echo.
echo PMSM drive test failed with exit code %RESULT%.
if "%NO_PAUSE%"=="0" pause
exit /b %RESULT%
