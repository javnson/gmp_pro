...
// Generated by GMP mna_solver/cpp_codegen.py from BUCK_NPC.CIR.
// Source SHA-256: 511c757a4db22b602554e5aa2bb89772cf990025ffb4aeb97be8448b356f8c13
// Matrix equivalence tolerance: 9.9999999999999998e-13
// Matrix backend: eigen
// Selection states: 324; stored reachable states: 324; unique calculation states: 324.
...
class BuckNpcCircuit {
public:
    static constexpr std::size_t state_count = 7;
    static constexpr std::size_t signal_count = 15;
    static constexpr std::size_t analog_input_count = 2;
    static constexpr std::size_t pwm_input_count = 4;
    static constexpr std::size_t topology_count = 324;
    static constexpr std::size_t stored_topology_count = 324;
    static constexpr std::size_t calculation_state_count = 324;
    static constexpr const char* matrix_backend = "eigen";
    static constexpr const char* matrix_storage = "archive";
    static constexpr const char* archive_filename = "bucknpccircuit.archive";
    static constexpr std::size_t state_matrix_count = 648;
    static constexpr std::size_t input_matrix_count = 648;
    static constexpr std::size_t state_vector_count = 617;
    ...
    static constexpr double normal_step_s = 1.0000000000000001e-09;
    static constexpr double short_step_s = 1e-10;
    ...
    struct Outputs {
        ...
        double operator[](std::string_view name) const {
            if (name == "V(4)" || name == "n_4") return n_4;
    ...
    const Outputs& step_short(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, double VS2, double VS1) {
        return step_short(Inputs{PWM1, PWM2, PWM3, PWM4, VS2, VS1});
    }

    const Outputs& step_normal(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, double VS2, double VS1) {
        return step_normal(Inputs{PWM1, PWM2, PWM3, PWM4, VS2, VS1});
    }

    const Outputs& run(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, double VS2, double VS1) {
        return step_normal(PWM1, PWM2, PWM3, PWM4, VS2, VS1);
    }

    const Outputs& operator()(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, double VS2, double VS1) {
        return run(PWM1, PWM2, PWM3, PWM4, VS2, VS1);
    }
...
private:
    using StateMatrix = Eigen::Matrix<double, 7, 7>;
    using InputMatrix = Eigen::Matrix<double, 7, 2>;
    using StateVector = Eigen::Matrix<double, 7, 1>;
    using SignalMatrix = Eigen::Matrix<double, 15, 7>;
    using SignalInputMatrix = Eigen::Matrix<double, 15, 2>;
    using SignalVector = Eigen::Matrix<double, 15, 1>;
    using InputVector = Eigen::Matrix<double, 2, 1>;
    ...
    static std::shared_ptr<const ArchiveData> load_archive(
        ...
        constexpr std::array<std::uint8_t, 32U> expected_source_hash{{
            0x4aU, 0x0bU, 0x41U, 0x82U, 0x50U, 0x51U, 0x4cU, 0xd1U, 0xd1U, 0x9cU, 0x12U, 0x7eU, 0x7bU, 0x96U, 0xe2U, 0xc0U, 0x3dU, 0xd0U, 0x4eU, 0xe2U, 0xebU, 0xd9U, 0x04U, 0x4fU, 0xcaU, 0x0aU, 0x1fU, 0x76U, 0x56U, 0x58U, 0xffU, 0x8cU
        }};
    ...
    std::size_t select_topology(const Inputs& inputs) {
        ...
        if (inputs.PWM3 != 0U) {
            ...
            const double reverse_voltage_0 = signals_(8) - signals_(7);
        ...
        if (inputs.PWM4 != 0U) {
            ...
            const double reverse_voltage_1 = signals_(10) - signals_(9);
        ...
        if (inputs.PWM1 != 0U) {
            ...
            const double reverse_voltage_2 = signals_(12) - signals_(11);
        ...
        if (inputs.PWM2 != 0U) {
            ...
            const double reverse_voltage_3 = signals_(14) - signals_(13);
        ...
        const double diode_voltage_0 = signals_(3) - signals_(4);
        ...
        const double diode_voltage_1 = signals_(5) - signals_(6);
    ...
    const Outputs& step(const Inputs& inputs, bool use_short_step) {
        ...
        input_vector << inputs.VS2, inputs.VS1;
        if (use_short_step) {{
        ...
        output.n_4 = signals_(0);
        output.VAM1 = signals_(1);
        output.VF1 = signals_(2);
        return output;
    }
    ...
    std::array<bool, 4> body_on_{};
    std::array<bool, 2> diode_on_{};
