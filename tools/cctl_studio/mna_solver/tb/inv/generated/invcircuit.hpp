...
// Generated by GMP mna_solver/cpp_codegen.py from INV.CIR.
// Source SHA-256: 693608f7a8c04de96e2f749bde052b2e02ea05370c9292493369df26c38e2f73
...
class InvCircuit {
public:
    static constexpr std::size_t state_count = 10;
    static constexpr std::size_t signal_count = 18;
    ...
    static constexpr const char* archive_filename = "invcircuit.archive";
    static constexpr std::size_t state_matrix_count = 1458;
    static constexpr std::size_t input_matrix_count = 1433;
    static constexpr std::size_t state_vector_count = 1316;
    static constexpr std::size_t signal_matrix_count = 78;
    static constexpr std::size_t signal_input_matrix_count = 1;
    static constexpr std::size_t signal_vector_count = 5;
    ...
    struct Outputs {
        ...
        double operator[](std::string_view name) const {
            ...
            if (name == "V(5,1)" || name == "n_5") return n_5;
            if (name == "V(4,1)" || name == "n_4") return n_4;
            if (name == "V(3,1)" || name == "n_3") return n_3;
    ...
    const Outputs& step_short(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, std::uint32_t PWM5, std::uint32_t PWM6, double VS1) {
        return step_short(Inputs{PWM1, PWM2, PWM3, PWM4, PWM5, PWM6, VS1});
    }

    const Outputs& step_normal(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, std::uint32_t PWM5, std::uint32_t PWM6, double VS1) {
        return step_normal(Inputs{PWM1, PWM2, PWM3, PWM4, PWM5, PWM6, VS1});
    }

    const Outputs& run(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, std::uint32_t PWM5, std::uint32_t PWM6, double VS1) {
        return step_normal(PWM1, PWM2, PWM3, PWM4, PWM5, PWM6, VS1);
    }

    const Outputs& operator()(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, std::uint32_t PWM5, std::uint32_t PWM6, double VS1) {
        return run(PWM1, PWM2, PWM3, PWM4, PWM5, PWM6, VS1);
    }
...
private:
    using StateMatrix = Eigen::Matrix<double, 10, 10>;
    using InputMatrix = Eigen::Matrix<double, 10, 1>;
    using StateVector = Eigen::Matrix<double, 10, 1>;
    using SignalMatrix = Eigen::Matrix<double, 18, 10>;
    using SignalInputMatrix = Eigen::Matrix<double, 18, 1>;
    using SignalVector = Eigen::Matrix<double, 18, 1>;
    ...
    static std::shared_ptr<const ArchiveData> load_archive(
        ...
        constexpr std::array<std::uint8_t, 32U> expected_source_hash{{
            0xecU, 0x9aU, 0xdcU, 0xf1U, 0x42U, 0xa5U, 0x6fU, 0xbeU, 0x14U, 0x6cU, 0xb9U, 0x39U, 0x49U, 0xd7U, 0x1eU, 0xcaU, 0x02U, 0xe5U, 0x66U, 0xd6U, 0xe6U, 0x19U, 0xe9U, 0xa0U, 0x9bU, 0x3cU, 0xfaU, 0x37U, 0x74U, 0xcdU, 0x28U, 0xa7U
        }};
    ...
    std::size_t select_topology(const Inputs& inputs) {
        ...
        if (inputs.PWM3 != 0U) {
            ...
            const double reverse_voltage_0 = signals_(7) - signals_(6);
        ...
        if (inputs.PWM4 != 0U) {
            ...
            const double reverse_voltage_1 = signals_(9) - signals_(8);
        ...
        if (inputs.PWM1 != 0U) {
            ...
            const double reverse_voltage_2 = signals_(11) - signals_(10);
        ...
        if (inputs.PWM2 != 0U) {
            ...
            const double reverse_voltage_3 = signals_(13) - signals_(12);
        ...
        if (inputs.PWM5 != 0U) {
            ...
            const double reverse_voltage_4 = signals_(15) - signals_(14);
        ...
        if (inputs.PWM6 != 0U) {
            ...
            const double reverse_voltage_5 = signals_(17) - signals_(16);
    ...
    const Outputs& step(const Inputs& inputs, bool use_short_step) {
        ...
        output.VAM3 = signals_(0);
        output.VAM2 = signals_(1);
        output.VAM1 = signals_(2);
        output.n_5 = signals_(3);
        output.n_4 = signals_(4);
        output.n_3 = signals_(5);
        return output;
    }
