...
// Generated by GMP mna_solver/cpp_codegen.py from FSBB.CIR.
// Source SHA-256: 91e7dada1f1a56dc62b4602178783f5b674c156441b69da006f97bda1a9f31cd
// Matrix equivalence tolerance: 9.9999999999999998e-13
// Matrix backend: eigen
// Selection states: 81; stored reachable states: 81; unique calculation states: 81.
...
class FsbbCircuit {
public:
    static constexpr std::size_t state_count = 5;
    ...
    static constexpr std::size_t topology_count = 81;
    static constexpr std::size_t stored_topology_count = 81;
    static constexpr std::size_t calculation_state_count = 81;
    static constexpr const char* matrix_backend = "eigen";
    static constexpr const char* matrix_storage = "archive";
    static constexpr const char* archive_filename = "fsbbcircuit.archive";
    static constexpr std::size_t state_matrix_count = 162;
    static constexpr std::size_t input_matrix_count = 106;
    static constexpr std::size_t state_vector_count = 129;
    ...
    const Outputs& step_short(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, double VS1) {
        return step_short(Inputs{PWM1, PWM2, PWM3, PWM4, VS1});
    }

    const Outputs& step_normal(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, double VS1) {
        return step_normal(Inputs{PWM1, PWM2, PWM3, PWM4, VS1});
    }

    const Outputs& run(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, double VS1) {
        return step_normal(PWM1, PWM2, PWM3, PWM4, VS1);
    }

    const Outputs& operator()(std::uint32_t PWM1, std::uint32_t PWM2, std::uint32_t PWM3, std::uint32_t PWM4, double VS1) {
        return run(PWM1, PWM2, PWM3, PWM4, VS1);
    }
...
private:
    ...
    using InputMatrix = Eigen::Matrix<double, 5, 1>;
    using StateVector = Eigen::Matrix<double, 5, 1>;
    using SignalMatrix = Eigen::Matrix<double, 10, 5>;
    ...
    static std::shared_ptr<const ArchiveData> load_archive(
        ...
        constexpr std::array<std::uint8_t, 32U> expected_source_hash{{
            0x56U, 0x26U, 0x8dU, 0x47U, 0x2eU, 0xb0U, 0x07U, 0x76U, 0x77U, 0x77U, 0x52U, 0x80U, 0xb3U, 0x76U, 0x2dU, 0x8fU, 0x46U, 0x67U, 0x9dU, 0x05U, 0x74U, 0x84U, 0x6bU, 0x1eU, 0x51U, 0x55U, 0x9cU, 0x6aU, 0x07U, 0xe2U, 0xddU, 0x9eU
        }};
    ...
    std::size_t select_topology(const Inputs& inputs) {
        ...
        if (inputs.PWM1 != 0U) {
            ...
            const double reverse_voltage_0 = signals_(3) - signals_(2);
        ...
        if (inputs.PWM2 != 0U) {
            ...
            const double reverse_voltage_1 = signals_(5) - signals_(4);
        ...
        if (inputs.PWM4 != 0U) {
            ...
            const double reverse_voltage_2 = signals_(7) - signals_(6);
        ...
        if (inputs.PWM3 != 0U) {
            ...
            const double reverse_voltage_3 = signals_(9) - signals_(8);
