...
echo [2/5] Running unit and supplied-netlist tests...
python -m unittest discover -s "%SOLVER_DIR%tests" -v
if errorlevel 1 goto :failed

echo [3/5] Checking discrete-equation generation...
python "%SOLVER_DIR%mna_solver.py" discretize "%SOLVER_DIR%tb\example6.cir" --dt 1N >nul
if errorlevel 1 goto :failed

echo [4/5] Checking time-domain CSV generation...
...
echo [5/5] Checking frequency-response CSV generation...
