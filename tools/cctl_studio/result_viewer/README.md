...
Launch `run_result_viewer.bat`, optionally passing a result filename. Each plot has an independent curve set. Enable linked X zoom to align only the time axes, linked Y zoom to align only the vertical ranges, or both when full synchronization is wanted. See `README_CN.md` for the complete workflow.
