...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 让网桥吐出的数据流，直接灌进仿真引擎的 UI 更新函数里
        self.tab_pil_bridge.sig_rx_parsed.connect(self.tab_sim.update_rx_ui_from_bridge)

        # 让 Tunable 的状态锁，直接控制 PIL 的发送节流阀 (现在只需控制 GUI 节流，底层已被优先级队列接管)
    ...
    def _build_serial_panel(self, layout: QVBoxLayout):
        ...
        self.cb_ports.view().setMinimumWidth(350) # 突破框宽度，显示超长设备名
    ...
    # =========================================================
    # 安全守护：重写关闭事件，切断底层双核调度线程
    # =========================================================
    def closeEvent(self, event):
        """窗口关闭时的安全清理机制，防止出现残留的幽灵进程占用串口"""
        self.log_message("⏳ 正在安全关闭通信内核引擎...")
        
        # 释放所有子页面的内部资源（如 Tunable 的定时器等）
