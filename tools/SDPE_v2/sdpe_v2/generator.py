...
class HeaderGenerator:
    ...
    def _append_parameter_macros(
        ...
        for pname, pspec in schema.parameters.items():
            ...
            self._append_config_macro(
                lines,
                macro,
                self._format_parameter_value(value, pspec.value_format, entity, prefix, pspec.numeric_domain),
        ...
        for pname, pspec in child_schema.parameters.items():
            ...
            if found_override:
                value = self._format_parameter_value(
                    override_value, pspec.value_format, parent, parent_prefix, pspec.numeric_domain
                )
            else:
    ...
    def _format_parameter_value(
        self, value: Any, value_format: str, entity: HardwareEntity, prefix: str, numeric_domain: str = "raw"
    ) -> str:
        formatted: str
        if isinstance(value, dict):
            if "literal" in value:
                formatted = str(value["literal"])
            elif "macro" in value:
                formatted = str(value["macro"])
            elif "ref" in value:
                formatted = self._resolve_entity_symbol(entity, prefix, str(value["ref"]))
            elif "export" in value:
                formatted = self._resolve_entity_symbol(entity, prefix, str(value["export"]))
            elif "expr" in value:
                formatted = self._format_local_expr(str(value["expr"]), entity, prefix)
            else:
                raise SDPEError(f"Unsupported parameter value object: {value!r}")
        elif isinstance(value, str):
            match = re.fullmatch(r"\$\{([A-Za-z0-9_][A-Za-z0-9_.]*)\}", value.strip())
            if match:
                formatted = self._resolve_entity_symbol(entity, prefix, match.group(1))
            else:
                formatted = c_literal(value, value_format)
        else:
            formatted = c_literal(value, value_format)
        return self._apply_numeric_domain(formatted, numeric_domain)

    def _apply_numeric_domain(self, value: str, numeric_domain: str) -> str:
        """Wrap a generated C expression at its declared numeric boundary."""

        domain = numeric_domain.strip().lower()
        if domain == "raw":
            return value
        if domain == "real":
            return value
        if domain == "parameter":
            return f"real2param({value})"
        if domain == "ctrl":
            return f"real2ctrl({value})"
        raise SDPEError(f"Unsupported numeric domain: {numeric_domain}")
        ...
        for pname, pspec in schema.parameters.items():
            ...
            if not found:
                if pspec.default is None:
                    continue
                value = pspec.default
            formatted = self._format_parameter_value(
                value, pspec.value_format, entity, prefix, pspec.numeric_domain
            )
