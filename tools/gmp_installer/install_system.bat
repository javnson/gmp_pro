...
echo [SETUP] Auditing facility registry dependencies...
python "%GMP_PRO_LOCATION%\tools\facilities_generator\src_mgr\facility_dependency_audit.py" --repo "%GMP_PRO_LOCATION%"
if errorlevel 1 exit /b 1

echo [SETUP] Distributing source-management tools...
python "%GMP_PRO_LOCATION%\tools\facilities_generator\src_mgr\framework_distribute_tools_v3.py" --deploy-only
if errorlevel 1 exit /b 1
